...
The Microsoft Dynamics 365 Project Timesheet aplicația mobilă permite utilizatorilor să trimită și să aprobe foile de pontaj pentru proiecte pe dispozitivul lor mobil (iPhone sau Android). Această aplicație mobilă prezintă funcționalitatea foii de pontaj care se află în zona de management al proiectelor și contabilitate a Dynamics 365 Finance. Ajută la îmbunătățirea productivității și eficienței utilizatorilor și, de asemenea, permite introducerea și aprobarea în timp util a foilor de pontaj ale proiectului.
...
### <a name="resolve-sign-in-issues"></a>Rezolvați problemele de conectare

**Problema:** În timpul conectării la aplicația Project Timesheet Mobile, utilizatorii primesc un mesaj de eroare care afirmă că „nu pot accesa aplicația”2bc50526-cdc3-4e36-a970-c284c34cbd6e ' în acel chiriaș."

**Problema:** În timpul conectării la aplicația Project Timesheet Mobile, utilizatorii primesc o eroare care seamănă cu unul dintre următoarele exemple:

- „AADSTS50020: Contul de utilizator „[nume utilizator]” de la furnizorul de identitate „https://sts.windows.net/ [app id]” nu există în chiriașul „[tenant id]” și nu poate accesa aplicația „[app id]” din acel chiriaș.”
- „Contul de utilizator selectat nu există în chiriașul „[tenant id]” și nu poate accesa aplicația „[app id]” din acel chiriaș.”

**Explicaţie:** Aceste probleme sunt cauzate de o modificare care a fost făcută în Azure Active Directory (Azure AD) în mai 2022 și care are legătură cu utilizatorii externi. Deoarece această modificare nu a fost făcută aplicațiilor de finanțare și operațiuni, poate afecta clienții de pe orice versiune a platformei sau a aplicației.

**Remediere:** Toți utilizatorii externi trebuie să fie invitați la chiriaș prin intermediul Azure AD. Pentru mai multe informații, vezi [Invitați utilizatori cu Azure Active Directory Colaborare B2B](/power-platform/admin/invite-users-azure-active-directory-b2b-collaboration).
...
În **Proiect** câmp, selectați proiectul pentru care introduceți timpul. Câmpul **Client** este completat automat.
...
Câmpul **Proprietate de linie** va fi setat la o valoare implicită, pe baza parametrilor de management și contabilitate a proiectului. Când parametrii proiectului/categoriei și categoriei/resurselor sunt activate, valoarea **Proprietate de linie** va fi setată la valoarea implicită pe care ați definit-o pentru această validare. Când parametrii proiectului/categoriei și categoriei/resursei nu sunt activați, **Proprietatea liniei** valoarea va fi implicită în conformitate cu **Activați proprietatea implicită a liniei** câmp de pe **Management de proiect și parametri contabili** pagină. Valoarea **Proprietate de linie** poate fi suprascrisă.

Selectați o zi pentru a adăuga timp. Introduceți numărul de ore pe care le-ați lucrat în fiecare zi.

Pentru a adăuga comentarii despre orele pe care le introduceți, faceți clic **Adaugă comentarii**, apoi introduceți comentarii pentru un public intern, un public de clienți sau ambele.
...
Dimensiunea financiară și suportul pentru atașamente nu sunt furnizate în aplicația mobilă.
